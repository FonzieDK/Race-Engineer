...
test("car status moves to setup while overview keeps its original grid space", () => {
  ...
  assert.match(styles, /\.car-setup-layout\s*\{[^}]*grid-template-columns:\s*repeat\(3, minmax\(0, 1fr\)\)[^}]*align-items:\s*stretch/s);
  assert.match(styles, /\.charts-grid\.is-car-status-relocated \.overview-car-status-placeholder\s*\{[^}]*aspect-ratio:\s*0\.92/s);
  assert.match(styles, /\.car-setup-window\s*\{[^}]*height:\s*100%/s);
  assert.match(styles, /\.car-status-module \.section-label,\s*\.car-setup-window \.section-label\s*\{[^}]*font-size:\s*0\.82rem/s);
});
...
test("car setup follows the track map viewport sizing rules", () => {
  ...
  assert.match(styles, /\.car-setup-layout\s*\{[^}]*height:\s*100%[^}]*min-height:\s*100%[^}]*grid-template-columns:\s*repeat\(3, minmax\(0, 1fr\)\)[^}]*grid-template-rows:\s*minmax\(0, 1fr\)[^}]*overflow:\s*hidden/s);
...
test("pit setup draft covers timing, fuel, tyres and optional service", () => {
  assert.match(html, /class="[^"]*pit-setup-preview[^"]*"[\s\S]*Next stop[\s\S]*Pit window[\s\S]*Est\. stop/);
  assert.match(html, /id="pit-fuel-add"[\s\S]*Fuel at pit entry[\s\S]*Estimated range/);
  assert.doesNotMatch(html, /pit-tire-change-all|CHANGE ALL TIRES/);
  assert.match(html, /id="pit-fast-repair" type="checkbox" hidden[\s\S]*id="pit-windscreen-tearoff" type="checkbox" checked hidden/);
  ...
  assert.match(html, /Total pit-lane loss[\s\S]*Apply pit setup/);
...
test("fuel calculation keeps pit inputs without the optional service card", () => {
  const pitSetupMarkup = html.slice(
    html.indexOf('aria-label="Pit setup"'),
    html.indexOf('<div class="cumulation-stack">'),
  );
  assert.match(html, /aria-label="Fuel calculation"[\s\S]*?>FUEL CAL<\/p>[\s\S]*?class="car-setup-window-content fuel-cal-window-content"[\s\S]*?id="pit-fast-repair"[\s\S]*?id="pit-windscreen-tearoff"[\s\S]*?class="pit-plan-footer"[\s\S]*?Total pit-lane loss[\s\S]*?Apply pit setup/);
  ...
  assert.match(styles, /\.fuel-cal-window-content\s*\{[^}]*align-content:\s*start[^}]*gap:\s*12px[^}]*padding:/s);
});
...
test("tire change controls are accessible wheel-specific toggles", () => {
  ...
  assert.doesNotMatch(script, /pitAllTiresToggleEl|wheel: "all"/);
  ...
  assert.match(script, /const pitTireChanges = telemetry\.pit_tire_changes \|\| \{\}[\s\S]*?button\.setAttribute\("aria-pressed", String\(selected\)\)[\s\S]*?classList\.remove\("has-command-error"\)/);
...
test("windscreen tear-off matches the original tire-change button size", () => {
  assert.match(html, /class="pit-car-service-row"[\s\S]*?id="pit-fast-repair-control"[\s\S]*?id="pit-car-windscreen-tearoff"[\s\S]*?>\s*<span>WINDSHIEL TEAROFF<\/span><\/button>[\s\S]*?class="pit-car-center"[\s\S]*?class="pit-car-plan"/);
...
test("compact wrench button sits left of windscreen tear-off", () => {
  assert.match(html, /class="pit-car-service-row"[\s\S]*?id="pit-fast-repair-toggle"[\s\S]*?<span>Repair<\/span>[\s\S]*?id="pit-car-windscreen-tearoff"/);
  assert.match(html, /id="pit-fast-repair-toggle-adjacent"[\s\S]*?<span>Repair<\/span>[\s\S]*?id="pit-car-windscreen-tearoff"/);
  ...
  assert.match(styles, /\.pit-car-service-row \.pit-fast-repair-control\s*\{[^}]*position:\s*absolute[^}]*top:\s*0[^}]*left:\s*25px[^}]*transform:\s*none/s);
  assert.match(styles, /\.pit-car-service-row \.pit-car-tearoff-toggle\s*\{[^}]*grid-column:\s*2[^}]*justify-self:\s*center/s);
  assert.match(html, /class="pit-car-plan"[\s\S]*?class="pit-fast-repair-control pit-fast-repair-control-bottom"[\s\S]*?id="pit-fast-repair-toggle-bottom"/);
  assert.equal((html.match(/data-pit-fast-repair-toggle/g) || []).length, 3);
  ...
  assert.match(styles, /\.pit-car-service-row \.pit-fast-repair-control-adjacent\s*\{[^}]*right:\s*25px[^}]*left:\s*auto/s);
  assert.match(script, /const pitFastRepairToggleEls = Array\.from\(document\.querySelectorAll\("\[data-pit-fast-repair-toggle\]"\)\)[\s\S]*?pitFastRepairToggleEls\.forEach\(\(button\) => \{[\s\S]*?button\.addEventListener\("click"[\s\S]*?button\.disabled[\s\S]*?pitFastRepairEl\.checked = !pitFastRepairEl\.checked/);
  assert.match(script, /function formatFastRepairsRemaining\(limit, used\)[\s\S]*?return "∞"/);
  assert.match(script, /telemetry\.fast_repairs_limit[\s\S]*?telemetry\.player_fast_repairs_used/);
  assert.match(script, /const hasNoFastRepairs = fastRepairsRemaining === "0"[\s\S]*?pitFastRepairToggleEls\.forEach[\s\S]*?button\.disabled = hasNoFastRepairs[\s\S]*?pitFastRepairEl\.disabled = hasNoFastRepairs/);
